+(function (scope) {
  ...
  function onConnect() {
    this._client.subscribe(this._options.device + TOPIC.PING);

    var message = new Paho.MQTT.Message("OK");
    message.destinationName = this._options.device + TOPIC.STATUS;
    message.qos = 0;
  ...
  function onMessage(message) {
    var data = message._getPayloadBytes();
    console.log("Mqtt onMessage:", data);
  ...
  function sendOut() {
    ...
    payload.destinationName = this._options.device + TOPIC.PONG;
    payload.qos = 0;
